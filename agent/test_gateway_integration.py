...
if __name__ == "__main__":
    # 這份檔案是需要真實平台 stack 在跑的手動整合測試腳本（見檔頭 docstring），
    # 不是給 pytest 收集用的，但檔名符合 test_*.py，pytest 掃到目錄時還是會
    # 嘗試 import 它。用這個 guard 讓 import 本身無副作用——不會解析
    # sys.argv（argparse 遇到 pytest 自己的命令列參數會直接丟
    # SystemExit(2)）、不會打真實網路請求、不會呼叫 sys.exit()。少了這個
    # guard，pytest import 這個模組時會在 collection 階段觸發 SystemExit，
    # 讓 pytest 整個 INTERNALERROR、後面所有測試都不會跑（同一個問題在
    # test_buffer.py 已經在 CI 實際發生過）。
    run_tests()
