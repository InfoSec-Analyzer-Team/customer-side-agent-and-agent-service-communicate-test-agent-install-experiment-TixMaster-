...
def run_tests() -> None:
    ...
    # ── Test G: 413 down to a single event → discarded, sibling sent ───────
    cleanup(); reset_mock()
    _MockHandler.responses = [
...
if __name__ == "__main__":
    ...
    # guard 讓 import 本身無副作用（不會啟動 mock HTTP server、不會跑測試、
    # 不會呼叫 sys.exit()），只有真的用 `python test_sender.py` 執行才會跑——
    # 少了這個 guard，pytest import 這個模組時會執行到底部的 sys.exit()，在
    # collection 階段觸發 SystemExit，讓 pytest 整個 INTERNALERROR、後面所有
    # 測試都不會跑（同一個問題在 test_buffer.py 已經在 CI 實際發生過）。
    run_tests()
