...
打真實存在的端點,payload 才會進到應用邏輯、status/size 才真實。打不存在的路徑會觸發你那個 SPA fallback(回 200/6537),status/size 就失真(如剛剛記錄的)。
時序自然分散:分時段、分批跑,別全擠在同一分鐘,否則 hour/time_period/is_odd_hour 在攻擊類別裡沒變異。用「真跑」解決,不要事後竄改 timestamp。